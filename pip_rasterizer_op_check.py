...
"""Used for testing rasterizer_op with pip package."""
import numpy as np
import six
import tensorflow as tf
from tensorflow_graphics.geometry.transformation import look_at
from tensorflow_graphics.rendering.camera import perspective
from tensorflow_graphics.rendering.opengl import rasterizer
...
world_to_camera = look_at.right_handed(camera_origin, look_at_point, camera_up)
perspective_matrix = perspective.right_handed(fov,
...
for idx in range(max_depth - min_depth):
  tris[idx, :] = (-100.0, 100.0, idx + min_depth, 100.0, 100.0, idx + min_depth,
                  0.0, -100.0, idx + min_depth)
...
render_parameters = {
    ...
    "triangular_mesh":
        ("buffer",
         tf.reshape(tris, shape=tf.concat(values=(batch_shape, (9,)), axis=0)))
}
